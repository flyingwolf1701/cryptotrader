...
from services.binance.restAPI import StakingOperations
...
def main():
    ...
    try:
        staking_assets = client.get_staking_asset_info(TEST_ASSET)
        ...
        if staking_assets:
            ...
            logger.info(f"{Fore.YELLOW}No staking information retrieved or authentication required")
    ...
    try:
        ...
        try:
            ...
            if staking_result:
                ...
                logger.warning(f"{Fore.YELLOW}Staking request failed - API credentials might be missing or invalid")
        except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if unstaking_result:
                ...
                logger.warning(f"{Fore.YELLOW}Unstaking request failed - API credentials might be missing or invalid")
        except Exception as e:
    ...
    try:
        staking_balance = client.get_staking_balance(TEST_ASSET)
        ...
        if staking_balance:
            ...
            logger.info(f"{Fore.YELLOW}No staking balance retrieved or authentication required")
    except Exception as e:
    ...
    try:
        ...
        staking_history = client.get_staking_history(
            asset=TEST_ASSET,
        ...
        if staking_history:
            logger.info(f"{Fore.GREEN}Retrieved {len(staking_history)} staking history records for {TEST_ASSET}")
            
            for i, record in enumerate(staking_history[:5]):  # Show up to 5 records
                record_time = datetime.fromtimestamp(record.initiatedTime / 1000).strftime('%Y-%m-%d %H:%M:%S')
            ...
            logger.info(f"{Fore.YELLOW}No staking history retrieved or authentication required")
    except Exception as e:
    ...
    try:
        ...
        rewards_history = client.get_staking_rewards_history(
            asset=TEST_ASSET,
            start_time=start_time,
            end_time=end_time,
            limit=10
        ...
        if rewards_history:
            logger.info(f"{Fore.GREEN}Retrieved staking rewards history for {TEST_ASSET}")
            ...
            if rewards_history.data:
                for i, reward in enumerate(rewards_history.data[:5]):  # Show up to 5 rewards
                    reward_time = datetime.fromtimestamp(reward.time / 1000).strftime('%Y-%m-%d %H:%M:%S')
            ...
            logger.info(f"{Fore.YELLOW}No staking rewards history retrieved or authentication required")
    except Exception as e:
    ...
    logger.info(f"{Fore.YELLOW}This diagnostic primarily tested API connectivity and structure.")
