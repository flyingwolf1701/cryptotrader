...
from services.binance.models import OcoOrderResponse
...
async def get_oco_order(
    ...
    id_info = f"orderListId={order_list_id}" if order_list_id else f"origClientOrderId={orig_client_order_id}"
    logger.debug(f"Sent get OCO order request ({id_info}) with ID: {msg_id}")
    return msg_id

async def process_get_oco_order_response(response: Dict[str, Any]) -> Optional[OcoOrderResponse]:
    """
