...
async def stopUserDataStream(
    ...
    logger.debug(
        f"Sent stop user data stream request for listen key {listen_key[:10]}... with ID: {msg_id}"
    )
    return msg_id


async def processStopUserDataStreamResponse(response: Dict[str, Any]) -> bool:
    """
    ...
    try:
        ...
        if "error" in response:
            ...
            logger.error(
                f"Stop user data stream failed with code {error_code}: {error_msg}"
            )
