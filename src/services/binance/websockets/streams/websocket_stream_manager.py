...
class BinanceStreamManager:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self.subscribed_streams and (
                (self.use_combined_stream and "streams" not in url) or
                (not self.use_combined_stream and "stream_name" not in url)
            ):
        ...
        if self.is_connected and self.use_combined_stream:
            ...
            try:
                ...
                if 'result' in response and isinstance(response['result'], list):
                    ...
                    logger.warning(f"Unexpected LIST_SUBSCRIPTIONS response: {response}")
    ...
    async def _send_subscription_request(self, streams: List[str], subscribe: bool) -> bool:
        """
            ...
            try:
                ...
                if self.websocket:
                    ...
                    while asyncio.get_running_loop().time() < pong_wait_until:
                        if (asyncio.get_running_loop().time() - self.last_message_time < self.pong_timeout or 
                    ...
                    if (self.is_connected and 
                        asyncio.get_running_loop().time() - self.last_message_time > self.pong_timeout):
                ...
                if message:
                    try:
                        ...
                        # Handle stream data messages
                            ...
                            if self.use_combined_stream and 'stream' in data and 'data' in data:
                                ...
                                if self.subscribed_streams:
                                    ...
                                    logger.warning("Received data but no subscribed streams")
    ...
    async def _monitor_connection_age(self):
        ...
        while self.is_connected and not self.is_closing:
            try:
                ...
                current_time = asyncio.get_running_loop().time()
                if self.connection_start_time and current_time - self.connection_start_time > 23.5 * 60 * 60:
...
async def create_market_stream(
    symbols: List[str], 
    channels: List[str],
