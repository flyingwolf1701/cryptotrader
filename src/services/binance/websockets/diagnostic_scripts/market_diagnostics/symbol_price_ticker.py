"""
Binance Symbol Price Ticker Diagnostic Script
-------------------------------------------
Tests the Binance Symbol Price Ticker WebSocket API functionality to verify proper operation.

Usage:
    To run this script from the project root directory:
    python src/cryptotrader/services/binance/websocketAPI/diagnostic_scripts/market_diagnostics/symbol_price_ticker_diagnostic.py
"""
    ...
    try:
        ...
        if response_received:
            ...
            if tickers and isinstance(tickers, list):
                ...
                format_str = f"  {{:<{max_symbol_len}}}  {{:>12}}"
                logger.info(format_str.format("SYMBOL", "PRICE"))
                logger.info(format_str.format("-" * max_symbol_len, "-" * 12))
                for ticker in tickers:
                    logger.info(format_str.format(ticker.symbol, ticker.price))
            else:
