...
async def main():
    ...
    try:
        ...
        # Send request
        msg_id = await get_book_ticker(connection=connection, symbol=TEST_SYMBOL)
        ...
        if response_received:
            ...
            if ticker:
                print_test_result(
                    True, f"Successfully retrieved order book ticker for {TEST_SYMBOL}"
                )
        ...
        print_test_header(
            f"Getting order book tickers for multiple symbols: {TEST_SYMBOLS}"
        )
        response_received = False

        # Send request
        msg_id = await get_book_ticker(connection=connection, symbols=TEST_SYMBOLS)
        ...
        if response_received:
            ...
            if tickers and isinstance(tickers, list):
                print_test_result(
                    True,
                    f"Successfully retrieved order book tickers for {len(tickers)} symbols",
                )

                # Display all tickers in a table format
                logger.info(f"Current order book top:")
                logger.info(
                    f"  {'SYMBOL':<10}  {'BID PRICE':>12}  {'BID QTY':>10}  {'ASK PRICE':>12}  {'ASK QTY':>10}  {'SPREAD %':>10}"
                )
                logger.info(
                    f"  {'-' * 10}  {'-' * 12}  {'-' * 10}  {'-' * 12}  {'-' * 10}  {'-' * 10}"
                )
                ...
                for ticker in tickers:
                    spread = ticker.askPrice - ticker.bidPrice
                    spread_pct = (
                        (spread / ticker.bidPrice) * 100 if ticker.bidPrice > 0 else 0
                    )
                    logger.info(
                        f"  {ticker.symbol:<10}  {ticker.bidPrice:>12}  {ticker.bidQty:>10.4f}  "
                        f"{ticker.askPrice:>12}  {ticker.askQty:>10.4f}  {spread_pct:>10.4f}%"
                    )

                # Analyze the data
                spread_percentages = [
                    (t.askPrice - t.bidPrice) / t.bidPrice * 100 for t in tickers
                ]
