...
from services.binance.models import Candle, KlineInterval
...
async def get_klines_ws(
    ...
    msg_id = await connection.send(
        method="klines", params=params, security_type=SecurityType.NONE
    )

    logger.debug(
        f"Sent klines request for {symbol} with interval {interval}, ID: {msg_id}"
    )
    return msg_id
    ...
    try:
        ...
        for kline_data in result:
            ...
            candle = Candle(
                ...
                openPrice=float(kline_data[1]),  # Open price
                highPrice=float(kline_data[2]),  # High price
                lowPrice=float(kline_data[3]),  # Low price
                closePrice=float(kline_data[4]),  # Close price
                volume=float(kline_data[5]),  # Volume
                quoteVolume=float(kline_data[7]),  # Quote asset volume
            )
