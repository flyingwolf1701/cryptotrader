...
async def getRollingWindowStatsWS(
    ...
    msg_id = await connection.send(
        method="ticker", params=params, security_type=SecurityType.NONE
    )

    logger.debug(
        f"Sent rolling window ticker request with window {window_size}, ID: {msg_id}"
    )
    return msg_id


async def process_rolling_window_response(
    response: Dict[str, Any], ticker_type: Optional[str] = "FULL"
) -> Optional[
    Union[
        RollingWindowStats,
        RollingWindowStatsMini,
        List[Union[RollingWindowStats, RollingWindowStatsMini]],
    ]
]:
    """
