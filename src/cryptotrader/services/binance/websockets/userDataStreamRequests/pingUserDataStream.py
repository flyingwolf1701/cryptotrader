...
async def pingUserDataStream(
    ...
    logger.debug(
        f"Sent ping user data stream request for listen key {listen_key[:10]}... with ID: {msg_id}"
    )
    return msg_id


async def process_pingUserDataStreamResponse(response: Dict[str, Any]) -> bool:
    """
    ...
    try:
        ...
        if "error" in response:
            ...
            logger.error(
                f"Ping user data stream failed with code {error_code}: {error_msg}"
            )
