...
from cryptotrader.services.binance.models import AccountBalance
...
async def getAccountWS(
    ...
    msg_id = await connection.send(
        method="account.status", params=params, security_type=SecurityType.USER_DATA
    )
...
async def processAccountInfoResponse(
