...
from cryptotrader.services.binance.models.order_models import PreventedMatch

logger = get_logger(__name__)


async def getPreventedMatchesWS(
    ...
    if from_prevented_match_id is not None and prevented_match_id is not None:
        raise ValueError(
            "Cannot specify both from_prevented_match_id and prevented_match_id"
        )

    if from_prevented_match_id is not None and order_id is None:
        raise ValueError(
            "When using from_prevented_match_id, order_id must be specified"
        )
    ...
    if from_prevented_match_id is not None and not isinstance(
        from_prevented_match_id, int
    ):
    ...
    msg_id = await connection.send(
        method="myPreventedMatches", params=params, security_type=SecurityType.USER_DATA
    )
