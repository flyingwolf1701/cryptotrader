...
class UserOperations:
    ...
    def get_account(self, recv_window: Optional[int] = None) -> Optional[AccountBalance]:
        ...
        """
        request = self.request("GET", "/api/v3/account", weight=10).requires_auth(True)
    ...
    def get_account_status(self, recv_window: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        request = self.request("GET", "/sapi/v3/accountStatus", weight=1).requires_auth(True)
    ...
    def get_api_trading_status(self, recv_window: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        request = self.request("GET", "/sapi/v3/apiTradingStatus", weight=1).requires_auth(True)
    ...
    def get_asset_distribution_history(self, asset: Optional[str] = None, 
        ...
        """
        request = self.request("GET", "/sapi/v1/asset/assetDistributionHistory", weight=1).requires_auth(True)
        
        if asset is not None:
            request.with_query_params(asset=asset)
        if category is not None:
            request.with_query_params(category=category)
    ...
    def get_trade_fee(self, symbol: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        """
        request = self.request("GET", "/sapi/v1/asset/query/trading-fee", weight=1).requires_auth(True)
    ...
    def get_trading_volume(self) -> Optional[Dict[str, Any]]:
        ...
        """
        response = self.request("GET", "/sapi/v1/asset/query/trading-volume", weight=1).requires_auth(True).execute()
        return response