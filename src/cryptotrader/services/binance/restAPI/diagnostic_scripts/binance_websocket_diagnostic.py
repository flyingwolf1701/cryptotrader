...
class BinanceWebSocketTester:
    ...
    async def test_ping(self):
        ...
        try:
            ...
            print_test_result(
                success,
                "Ping response received" if success else "No ping response received",
            )
            return success
        except Exception as e:
    ...
    async def test_server_time(self):
        ...
        try:
            ...
            print_test_result(
                success,
                f"Received {self.messages_received} messages, {self.errors_received} errors",
            )

            if (
                self.last_message
                and "result" in self.last_message
                and "serverTime" in self.last_message["result"]
            ):
    ...
    async def test_rate_limits(self):
        ...
        try:
            ...
            msg_id = await self.connection.send(
                "exchangeInfo", {"symbols": [TEST_SYMBOL]}, return_rate_limits=True
            )
            ...
            print_test_result(
                success,
                "Rate limits received" if success else "No rate limits received",
            )
    ...
    async def test_authenticated_request(self):
        ...
        has_api_key = hasattr(Secrets, "BINANCE_API_KEY") and bool(
            getattr(Secrets, "BINANCE_API_KEY", None)
        )
        has_api_secret = hasattr(Secrets, "BINANCE_API_SECRET") and bool(
            getattr(Secrets, "BINANCE_API_SECRET", None)
        )
        ...
        try:
            ...
            msg_id = await self.connection.send(
                "account.status", {}, SecurityType.USER_DATA
            )
        ...
        for sig in (signal.SIGINT, signal.SIGTERM):
            try:
                loop.add_signal_handler(
                    sig, lambda: asyncio.create_task(tester.close())
                )
