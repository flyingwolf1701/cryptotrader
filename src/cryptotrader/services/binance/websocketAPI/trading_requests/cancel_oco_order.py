...
async def cancel_oco_order(
    ...
    Args:
        ...
        new_client_order_id: New client order ID for the canceled OCO
    ...
    id_info = f"orderListId={order_list_id}" if order_list_id else f"listClientOrderId={list_client_order_id}"
    logger.debug(f"Sent cancel OCO order request for {symbol} ({id_info}) with ID: {msg_id}")
    return msg_id

async def process_cancel_oco_order_response(response: Dict[str, Any]) -> Optional[OcoOrderResponse]:
    """
