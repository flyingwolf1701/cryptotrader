...
from cryptotrader.services.binance.models import OrderSide, TimeInForce, NewOrderResponseType, OcoOrderResponse
...
async def create_new_oco_order(
    ...
    Args:
        ...
        stop_price: Stop price for the stop order
    ...
    if stop_price is None and trailing_delta is None:
        raise ValueError("Either stopPrice or trailingDelta must be provided for OCO orders")
    
    # Validate stop limit order parameters
    if stop_limit_price is not None and stop_limit_time_in_force is None:
        raise ValueError("stopLimitTimeInForce is required when stopLimitPrice is provided")
    ...
    logger.debug(f"Sent create OCO order request for {symbol} {side.value} with ID: {msg_id}")
    return msg_id

async def process_create_oco_order_response(response: Dict[str, Any]) -> Optional[OcoOrderResponse]:
    """
