    ...
    try:
        ...
        if response_received:
            ...
            if candles and len(candles) > 0:
                logger.info(f"✓ Successfully retrieved {len(candles)} {INTERVAL} candlesticks")
                
                # Display first candle info
                first_candle = candles[0]
                logger.info(f"First candle details:")
                logger.info(f"  Timestamp: {datetime.fromtimestamp(first_candle.timestamp/1000)}")
                logger.info(f"  Open: {first_candle.open_price}")
                logger.info(f"  High: {first_candle.high_price}")
                logger.info(f"  Low: {first_candle.low_price}")
                logger.info(f"  Close: {first_candle.close_price}")
                ...
                logger.info(f"Price range in sample: {price_range} ({min_price} to {max_price})")
            else:
