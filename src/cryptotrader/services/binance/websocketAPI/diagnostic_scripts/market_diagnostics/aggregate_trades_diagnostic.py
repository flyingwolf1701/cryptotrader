"""
Binance Aggregate Trades Diagnostic Script
------------------------------------------
Tests the Binance Aggregate Trades WebSocket API functionality to verify proper operation.

Usage:
    To run this script from the project root directory:
    python src/cryptotrader/services/binance/websocketAPI/diagnostic_scripts/market_diagnostics/aggregate_trades_diagnostic.py
"""
...
async def main():
    """Run the aggregate trades diagnostic test"""
    ...
    try:
        ...
        # The get_aggregate_trades_ws function will use this connection
        ...
        print_test_header(f"Getting aggregate trades for {TEST_SYMBOL}")
        
        # Send request
        msg_id = await get_aggregate_trades_ws(
        ...
        # Process response
        if response_received:
            trades = await process_aggregate_trades_response(response_data)
            if trades and len(trades) > 0:
                logger.info(f"✓ Successfully retrieved {len(trades)} aggregate trades")
                ...
                logger.info(f"  Time: {datetime.fromtimestamp(first_trade.timestamp/1000)}")
            else:
