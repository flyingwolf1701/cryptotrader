...
async def main():
    ...
    try:
        ...
        # Process response
        if response_received:
            ticker = await process_24h_ticker_response(response_data, ticker_type="FULL")
            if ticker:
                print_test_result(True, f"Successfully retrieved 24hr ticker (FULL) for {TEST_SYMBOL}")
                logger.info(f"  Symbol: {ticker.symbol}")
                logger.info(f"  Last Price: {ticker.last_price}")
        ...
        # Process response
        if response_received:
            ticker = await process_24h_ticker_response(response_data, ticker_type="MINI")
            if ticker:
                print_test_result(True, f"Successfully retrieved 24hr ticker (MINI) for {TEST_SYMBOL}")
                logger.info(f"  Symbol: {ticker.symbol}")
                logger.info(f"  Last Price: {ticker.last_price}")
                logger.info(f"  24h High: {ticker.high_price}")
                logger.info(f"  24h Low: {ticker.low_price}")
        ...
        if response_received:
            ...
            if tickers and isinstance(tickers, list):
                print_test_result(True, f"Successfully retrieved 24hr tickers for {len(tickers)} symbols")
                
                # Display summary of all tickers
                logger.info(f"Summary of retrieved tickers:")
                for ticker in tickers:
                    logger.info(f"  {ticker.symbol}: Last Price: {ticker.last_price}, Change: {ticker.price_change} ({ticker.price_change_percent}%)")
            else:
