...
class SecurityType(str, Enum):
    ...
    USER_DATA = "USER_DATA"     # Private account information (requires API key and signature)
...
class BinanceWebSocketConnection:
    ...
    async def connect(self) -> bool:
        ...
        if self.ip_banned_until and time.time() * 1000 < self.ip_banned_until:
            ...
            logger.warning(f"IP banned, cannot connect for {ban_remaining_secs:.1f} more seconds")
            if self.on_error:
                await self.on_error(Exception(f"IP banned until {self.ip_banned_until}"))
            return False
    ...
    async def _monitor_connection_age(self):
        ...
        while self.is_connected and not self.is_closing:
            try:
                ...
                if self.connection_start_time and time.time() - self.connection_start_time > 23.5 * 60 * 60:
                    logger.info("Connection approaching 24-hour limit, initiating reconnection")
                    await self._reconnect()
                    return
    ...
    async def _reconnect(self):
        ...
        if self.ip_banned_until and time.time() * 1000 < self.ip_banned_until:
            ban_remaining_secs = (self.ip_banned_until - time.time() * 1000) / 1000
            logger.warning(f"IP banned, delaying reconnection for {ban_remaining_secs:.1f} seconds")
        ...
        # Check if we're rate limited
            ...
            logger.warning(f"Rate limited, delaying reconnection for {retry_secs:.1f} seconds")
        ...
        if 'error' in message:
            ...
            if status == 400:
                ...
                if self.retry_after:
                    ...
                    ban_duration_mins = (self.retry_after - time.time() * 1000) / (60 * 1000)
                    logger.error(f"IP banned until {self.retry_after} (approximately {ban_duration_mins:.1f} minutes)")
            elif status == 429:
                # Rate limit exceeded
    ...
    async def send(self, method: str, params: Optional[Dict[str, Any]] = None, 
        ...
        if self.ip_banned_until and time.time() * 1000 < self.ip_banned_until:
            ban_remaining_secs = (self.ip_banned_until - time.time() * 1000) / 1000
            raise Exception(f"IP banned, cannot send request for {ban_remaining_secs:.1f} more seconds")
        ...
        if self.retry_after and time.time() * 1000 < self.retry_after:
            ...
            raise Exception(f"Rate limited, retry after {retry_remaining_secs:.1f} more seconds")
