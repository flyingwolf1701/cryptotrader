...
class CandlestickChart:
    ...
    def __init__(self, parent, width=10, height=6, dpi=100):
        ...
        self.toolbar._message_label.config(background=Colors.BACKGROUND, foreground=Colors.FOREGROUND)
        for button in self.toolbar.winfo_children():
            if isinstance(button, tk.Button):
                button.config(background=Colors.BACKGROUND_LIGHT, foreground=Colors.FOREGROUND)
...
class ChartWidget(ttk.Frame):
    ...
    def init_ui(self):
        ...
        self.symbol_combo.bind("<<ComboboxSelected>>", lambda e: self.symbol_changed(self.symbol_var.get()))
        ...
        self.timeframe_combo = ttk.Combobox(timeframe_frame, textvariable=self.timeframe_var, width=5,
                                          values=["1m", "5m", "15m", "30m", "1h", "4h", "1d"])
        self.timeframe_combo.pack(padx=5, pady=5)
        self.timeframe_combo.bind("<<ComboboxSelected>>", lambda e: self.timeframe_changed(self.timeframe_var.get()))
        
        # Refresh button
        action_frame = ttk.LabelFrame(controls_frame, text="Actions")
        action_frame.pack(side=tk.LEFT, padx=5, pady=5, fill=tk.Y)
        
        self.refresh_btn = ttk.Button(action_frame, text="Refresh", command=self.refresh_chart)
